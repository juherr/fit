    ...
    class MusicPlayer
      ...
      def MusicPlayer.pause
        ...
        if (not @@playing.nil?) and @@paused.zero?
          @@paused = (Simulator.next_play_complete - Simulator.time) / 1000.0
      ...
      def MusicPlayer.seconds_remaining
        if not @@paused.zero?
          ...
          return (Simulator.next_play_complete - Simulator.time) / 1000.0
        else
          return 0
        end
      end
      ...
      def MusicPlayer.minutes_remaining
        (MusicPlayer.seconds_remaining / 0.6).round / 100.0
      end
