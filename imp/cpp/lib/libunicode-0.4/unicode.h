...
#include "tools/mandatory.h"  // Bring in CeeFIT decl specs
...
void ceefit_call_spec unicode_init (void);

/* Returns 1 if current locale uses UTF-8 charset.  If CHARSET is
   not null, sets *CHARSET to the name of the current locale's
   charset.  This value is statically allocated.  */
int  ceefit_call_spec unicode_get_charset (char **charset);

/* These are all analogs of the <ctype.h> functions.  */
int ceefit_call_spec  unicode_isalnum (unicode_char_t c);
int ceefit_call_spec  unicode_isalpha (unicode_char_t c);
int ceefit_call_spec  unicode_iscntrl (unicode_char_t c);
int ceefit_call_spec  unicode_isdigit (unicode_char_t c);
int ceefit_call_spec  unicode_isgraph (unicode_char_t c);
int ceefit_call_spec  unicode_islower (unicode_char_t c);
int ceefit_call_spec  unicode_isprint (unicode_char_t c);
int ceefit_call_spec  unicode_ispunct (unicode_char_t c);
int ceefit_call_spec  unicode_isspace (unicode_char_t c);
int ceefit_call_spec  unicode_isupper (unicode_char_t c);
int ceefit_call_spec  unicode_isxdigit (unicode_char_t c);
int ceefit_call_spec  unicode_istitle (unicode_char_t c);
int ceefit_call_spec  unicode_isdefined (unicode_char_t c);
int ceefit_call_spec  unicode_iswide (unicode_char_t c);

/* More <ctype.h> functions.  These convert between the three cases.
   See the Unicode book to understand title case.  */
unicode_char_t ceefit_call_spec  unicode_toupper (unicode_char_t c);
unicode_char_t ceefit_call_spec  unicode_tolower (unicode_char_t c);
unicode_char_t ceefit_call_spec  unicode_totitle (unicode_char_t c);

/* If C is a digit (according to `unicode_isdigit'), then return its
   numeric value.  Otherwise return -1.  */
int ceefit_call_spec  unicode_digit_value (unicode_char_t c);

/* If C is a hex digit (according to `unicode_isxdigit'), then return
   its numeric value.  Otherwise return -1.  */
int ceefit_call_spec  unicode_xdigit_value (unicode_char_t c);

/* Return the Unicode character type of a given character.  */
int ceefit_call_spec  unicode_type (unicode_char_t c);
...
char * ceefit_call_spec  unicode_previous_utf8 (const char *start, const char *p);
...
char * ceefit_call_spec  unicode_next_utf8 (const char *p);
...
int ceefit_call_spec  unicode_strlen (const char *p, int max);

/* Returns the visual width, in character-size units, of P, a string.
   This value may be used for tabulation.  */
int ceefit_call_spec  unicode_string_width (const char *p);
...
char * ceefit_call_spec  unicode_get_utf8 (const char *p, unicode_char_t *result);

/* Returns the offset within the string, in bytes, of the character offset
   given. */

size_t ceefit_call_spec  unicode_offset_to_index(const char *p, int offset);

/* Returns the offset within the string, in characters, of the byte offset
   given. */

size_t ceefit_call_spec  unicode_index_to_offset(const char *p, int offset);

/* Returns a pointer to the _last_ non-NULL utf-8 within the string */
	
char * ceefit_call_spec  unicode_last_utf8(const char *p);

/* Copies n characters from src to dest */

char * ceefit_call_spec  unicode_strncpy(char *dest, const char *src, size_t n);

/* Find the UTF-8 character corresponding to ch, in string p. These
   functions are equivilants to strchr and strrchr */

const char * ceefit_call_spec  unicode_strchr(const char *p, unicode_char_t ch);
const char * ceefit_call_spec  unicode_strrchr(const char *p, unicode_char_t ch);

/* Pads a string to fill out a requested visual width */

void ceefit_call_spec  unicode_pad_string(char *dest, int right, int width, const char *string);
	
/* Compute canonical ordering of a string in-place.  This rearranges
   decomposed characters in the string according to their combining
   classes.  See the Unicode manual for more information.  */
void ceefit_call_spec  unicode_canonical_ordering (unicode_char_t *string, size_t len);

/* Compute canonical decomposition of a character.  Returns malloc()d
   string of Unicode characters.  RESULT_LEN is set to the resulting
   length of the string.  */
unicode_char_t * ceefit_call_spec  unicode_canonical_decomposition (unicode_char_t ch,
						 size_t *result_len);
...
/* Create a new iconv conversion instance.  TOCODE is the destination
   ...
   ENOMEM or EINVAL.  */
unicode_iconv_t  ceefit_call_spec unicode_iconv_open (const char *tocode, const char *fromcode);

/* Close an iconv conversion instance.  */
int ceefit_call_spec  unicode_iconv_close (unicode_iconv_t cd);
...
size_t  ceefit_call_spec unicode_iconv (unicode_iconv_t cd,
