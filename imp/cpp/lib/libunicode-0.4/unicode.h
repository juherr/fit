...
void __cdecl unicode_init (void);

/* Returns 1 if current locale uses UTF-8 charset.  If CHARSET is
   not null, sets *CHARSET to the name of the current locale's
   charset.  This value is statically allocated.  */
int  __cdecl unicode_get_charset (char **charset);

/* These are all analogs of the <ctype.h> functions.  */
int __cdecl unicode_isalnum (unicode_char_t c);
int __cdecl unicode_isalpha (unicode_char_t c);
int __cdecl unicode_iscntrl (unicode_char_t c);
int __cdecl unicode_isdigit (unicode_char_t c);
int __cdecl unicode_isgraph (unicode_char_t c);
int __cdecl unicode_islower (unicode_char_t c);
int __cdecl unicode_isprint (unicode_char_t c);
int __cdecl unicode_ispunct (unicode_char_t c);
int __cdecl unicode_isspace (unicode_char_t c);
int __cdecl unicode_isupper (unicode_char_t c);
int __cdecl unicode_isxdigit (unicode_char_t c);
int __cdecl unicode_istitle (unicode_char_t c);
int __cdecl unicode_isdefined (unicode_char_t c);
int __cdecl unicode_iswide (unicode_char_t c);

/* More <ctype.h> functions.  These convert between the three cases.
   See the Unicode book to understand title case.  */
unicode_char_t __cdecl unicode_toupper (unicode_char_t c);
unicode_char_t __cdecl unicode_tolower (unicode_char_t c);
unicode_char_t __cdecl unicode_totitle (unicode_char_t c);

/* If C is a digit (according to `unicode_isdigit'), then return its
   numeric value.  Otherwise return -1.  */
int __cdecl unicode_digit_value (unicode_char_t c);

/* If C is a hex digit (according to `unicode_isxdigit'), then return
   its numeric value.  Otherwise return -1.  */
int __cdecl unicode_xdigit_value (unicode_char_t c);

/* Return the Unicode character type of a given character.  */
int __cdecl unicode_type (unicode_char_t c);
...
char * __cdecl unicode_previous_utf8 (const char *start, const char *p);
...
char * __cdecl unicode_next_utf8 (const char *p);
...
char * __cdecl unicode_get_utf8 (const char *p, unicode_char_t *result);

/* Returns the offset within the string, in bytes, of the character offset
   given. */

size_t __cdecl unicode_offset_to_index(const char *p, int offset);

/* Returns the offset within the string, in characters, of the byte offset
   given. */

size_t __cdecl unicode_index_to_offset(const char *p, int offset);

/* Returns a pointer to the _last_ non-NULL utf-8 within the string */
	
char * __cdecl unicode_last_utf8(const char *p);

/* Copies n characters from src to dest */

char * __cdecl unicode_strncpy(char *dest, const char *src, size_t n);

/* Find the UTF-8 character corresponding to ch, in string p. These
   functions are equivilants to strchr and strrchr */

char * __cdecl unicode_strchr(const char *p, unicode_char_t ch);
char * __cdecl unicode_strrchr(const char *p, unicode_char_t ch);

/* Pads a string to fill out a requested visual width */

void __cdecl unicode_pad_string(char *dest, int right, int width, const char *string);
	
/* Compute canonical ordering of a string in-place.  This rearranges
   decomposed characters in the string according to their combining
   classes.  See the Unicode manual for more information.  */
void __cdecl unicode_canonical_ordering (unicode_char_t *string, size_t len);

/* Compute canonical decomposition of a character.  Returns malloc()d
   string of Unicode characters.  RESULT_LEN is set to the resulting
   length of the string.  */
unicode_char_t * __cdecl unicode_canonical_decomposition (unicode_char_t ch,
						 size_t *result_len);
...
/* Create a new iconv conversion instance.  TOCODE is the destination
   ...
   ENOMEM or EINVAL.  */
unicode_iconv_t __cdecl unicode_iconv_open (const char *tocode, const char *fromcode);

/* Close an iconv conversion instance.  */
int __cdecl unicode_iconv_close (unicode_iconv_t cd);
...
size_t __cdecl unicode_iconv (unicode_iconv_t cd,
