...
public class TableSortCoordinator extends MouseAdapter {
  ...
  public void mouseReleased(MouseEvent e) {
    if (!e.isConsumed()) {
      ...
      TableColumnModel colModel = view.getColumnModel();
      ...
      if (modelIndex >= 0 && !isMoving(columnWidth, e.getX(), lastX)) {
        String headerName = view.getModel().getColumnName(modelIndex);
        ...
        for (int i = 0; i < colModel.getColumnCount(); i++) {
          ...
          String newTitle = (sortColumns.containsKey(identifier)) ? ((SortingEntry)sortColumns
              .get(identifier)).getHeaderTitle() : view.getModel().getColumnName(c.getModelIndex());
  ...
  /**
   * Distinction between resise, move and sort: If the mouse position has not been changed (much)
   * between press and release, it is not moving. I.e. "sorting" "much" means the difference between
   * the x-pos at press and release is less than a tenth of the width
   * 
   * @param columnWidth
   * @param x
   * @param y
