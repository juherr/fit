...
public class EditConfigurationDialog extends JDialog {
  ...
  /** resulting new configuration, if the user pressed the approve button.
   * null otherwise
   */
  ...
  public EditConfigurationDialog(Resources resources, Configuration config) {
    ...
    setLocationRelativeTo(resources.getApplicationFrame());
  ...
  private Configuration fields2config() {
    return new Configuration(inDir.getText(),outDir.getText(),libDir.getText(),pattern.getText(),runnerCommand.getText(),openCommand.getText(),editCommand.getText());
  }
